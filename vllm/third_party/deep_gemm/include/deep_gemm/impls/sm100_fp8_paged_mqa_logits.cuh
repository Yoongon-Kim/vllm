// LRoSA research fork: indexer logits = plain weighted head-sum (no ReLU).
// Delete the next line to restore DSA-native ReLU semantics.
            ...
            const auto reduce_and_store = [&](auto num_iters_c) {
                ...
                for (uint32_t i = 0; i < kNumIters; ++ i) {
                    ...
                    const auto transform = [&](const uint32_t& j, const float2& sum) {
                        // DG_MQA_LOGITS_NO_RELU (research fork): DSA's lightning
                        // indexer is TRAINED with the per-head ReLU; LRoSA-on-MLA's
                        // calibrated rotation targets the plain weighted head-SUM,
                        // which the ReLU breaks (true-attention coverage 0.91->0.79).
                        // Default here is NO-RELU (sum). REVERT the define below
                        // before running native-DSA models (DeepSeek V3.2 etc.).
